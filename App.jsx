...
import { getFirestore, doc, setDoc, onSnapshot, collection, query, orderBy, serverTimestamp, where, getDocs, deleteDoc, runTransaction } from 'firebase/firestore';
...
// The database path for public, shared vocabulary lists
...
const App = () => {
  ...
  const [activeView, setActiveView] = useState('dashboard'); // 'dashboard' or 'quiz'
      ...
      const handleSignIn = async () => {
        try {
          if (initialAuthToken) {
            ...
            // Use anonymous sign-in if no custom token is provided
  ...
  useEffect(() => {
    if (!db || !isAuthReady) {
      console.log('Waiting for DB or Auth to be ready...');
      return;
    }

    // Set up real-time listener for all vocab lists in the public collection
    const vocabColRef = collection(db, getCollectionPath(appId));
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setVocabLists(lists);
      setMessage(`Loaded ${lists.length} vocabulary list(s).`);
    }, (error) => {
      console.error("Firestore snapshot error:", error);
      setMessage('Error loading lists: Check console.');
    });
  ...
  const parseBulkInput = useCallback(() => {
    ...
    lines.forEach((line, index) => {
      // Normalize line to use colon as a separator for German and English
      ...
      if (parts.length >= 2) {
        cards.push({
          german: parts[0],
          english: parts[1],
          notes: parts.slice(2).join('; ') || '', // Optional notes/example sentences
          id: `${Date.now()}-${index}`
        });
      }
    });
  ...
  const saveList = async () => {
    ...
    const newDocRef = doc(collection(db, getCollectionPath(appId)));
    ...
    try {
      ...
      setMessage('Error saving list. See console.');
    }
  };

  const deleteList = async (listId, listName) => {
    if (!db || !confirm(`Are you sure you want to delete the list: "${listName}"?`)) return;

    try {
      await deleteDoc(doc(db, getCollectionPath(appId), listId));
  ...
  const nextCard = () => {
    ...
    // Optional: Re-shuffle when looping
  ...
  // Determines which side of the card is visible first
  ...
  const Dashboard = () => (
    <div className="p-4 space-y-8">
      ...
      <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-100">
        ...
        >
          {db ? `Save ${parseBulkInput().length} Cards` : 'Connecting to Database...'}
        </button>
      </div>
        ...
        {vocabLists.map((list) => (
          <div key={list.id} className="flex justify-between items-center bg-white p-4 rounded-xl shadow-lg border border-gray-100">
            <div>
              ...
              <div className="text-sm text-gray-500">{list.cardCount || list.cards.length} cards | Created by: {list.createdBy}</div>
            </div>
  ...
  const QuizView = () => (
    <div className="p-4 flex flex-col items-center justify-center min-h-screen-minus-header">
      {currentList && (
        <button 
